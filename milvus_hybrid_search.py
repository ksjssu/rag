...
def convert_sparse_to_dict(sparse_vector):
    """
    scipy의 sparse vector를 Milvus가 이해할 수 있는 dict 형식으로 변환합니다.
    Milvus가 기대하는 형식: {인덱스: 값, ...} 형태의 딕셔너리
    """
    print(f"\nSparse 벡터 타입: {type(sparse_vector)}")
    
    # 결과 딕셔너리
    result_dict = {}

    if hasattr(sparse_vector, 'todok'):
        # COO, CSR, CSC 등의 scipy sparse 형식 처리
        print("scipy sparse 행렬 타입 감지됨, todok() 사용")
        ...
        for key, value in dok.items():
            ...
            if isinstance(key, tuple):
                # (row, col) 형태
                col = key[1] if len(key) > 1 else key[0]
                result_dict[int(col)] = float(value)
            else:
                # 단일 인덱스
        ...
        # indices/values 형식인 경우 변환
        if "indices" in sparse_vector and "values" in sparse_vector:
            print("indices/values 딕셔너리를 인덱스:값 형태로 변환")
            indices = sparse_vector["indices"]
            values = sparse_vector["values"]
            
            for i, idx in enumerate(indices):
                if i < len(values):
                    result_dict[int(idx)] = float(values[i])
            
            return result_dict
        
        # 이미 올바른 {인덱스: 값} 형태인 경우
        elif all(isinstance(k, (int, np.integer)) and isinstance(v, (float, np.floating)) 
                for k, v in sparse_vector.items()):
            print("이미 올바른 인덱스:값 딕셔너리 형태")
            return {int(k): float(v) for k, v in sparse_vector.items()}
    
    # numpy/tensorflow/pytorch sparse 텐서
    elif hasattr(sparse_vector, 'indices') and hasattr(sparse_vector, 'values'):
        print("sparse 텐서(indices/values 속성) 감지됨")
        
        # 텐서의 indices와 values 속성 사용
        indices = sparse_vector.indices
        values = sparse_vector.values
        
        if hasattr(indices, "__len__") and hasattr(values, "__len__"):
            for i, idx in enumerate(indices):
                if i < len(values):
                    idx_val = idx
                    if hasattr(idx, "__len__") and len(idx) > 0:
                        idx_val = idx[-1]  # 다차원 인덱스의 경우 마지막 값 사용
                    result_dict[int(idx_val)] = float(values[i])
        
        return result_dict
        
    # 그 외 알려지지 않은 타입
    try:
        print(f"미지원 형식, 변환 시도: {sparse_vector}")
        
        # COO 행렬 처리
        if hasattr(sparse_vector, 'col') and hasattr(sparse_vector, 'data'):
            print("COO 행렬 변환 시도")
            for i, col in enumerate(sparse_vector.col):
                if i < len(sparse_vector.data):
                    result_dict[int(col)] = float(sparse_vector.data[i])
            
            return result_dict
            
        # nonzero 메서드 이용
        elif hasattr(sparse_vector, 'nonzero'):
            print("nonzero 메서드 사용 변환 시도")
            nonzero = sparse_vector.nonzero()
            ...
            if hasattr(sparse_vector, 'data'):
                data = sparse_vector.data
            else:
                # Dense에서 값 가져오기
                data = [sparse_vector[0, i] if sparse_vector.ndim > 1 else sparse_vector[i] for i in cols]
            
            for i, col in enumerate(cols):
                if i < len(data):
                    result_dict[int(col)] = float(data[i])
...
def sparse_search(col, query_sparse_embedding, limit=20, output_fields=None):
    """
...
def main():
    ...
    try:
        ...
        if hasattr(sparse_info, 'nnz'):
            print(f"Sparse vector 요소 수: {sparse_info.nnz}")
        ...
        # 2. Sparse 검색
        print("\n=== Sparse Vector 검색 결과 ===")
        sparse_results = sparse_search(col, query_embeddings["sparse"][0], limit=args.limit, output_fields=output_fields)
